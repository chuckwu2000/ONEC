...
# Based on the DF order, but consider the limited SRAM bank & bank size
class Weight_reuse_multi_bank_scheduler:
    def __init__(self, graph, need_allocate_tensors, same_layer_next_opids):
        ...
        # scheduled only update when the SRAM bank overuse or in the tail of schedule
                ...
                for opid in same_layer_opids:
                    ...
                    for tensor_id in op_info['inputs'][0:input_nums[input_idx]]:
                        ...
                        # The tensor may cross the block (in previous block in_DRAM := False), so need to set the tensor's in_DRAM
                            ...
                            # Then add the tensor to the SRAM
    ...
    class virtual_tensor_allocate:
        ...
        def virtual_tensor_allocate(self, opids, tensor_info):
            ...
            for tensor_id in sorted_tensor_ids:
                # Each tensor can always try to allocate from first bank
                prev_start_bank = 0
                best_start_bank = None
                smallest_gap = ArchitectureFeatures.SRAM_MAX_BANK
                bank_need = math.ceil(tensor_info[tensor_id].size / ArchitectureFeatures.SRAM_BANK_SIZE)
                ...
                for allocated_tensor in ordered_allocated_tensors:
                    ...
                    # Live range has overlap (try to find if there have the gap between this tensor's start address & previous start)
                    if max_first_op <= min_last_op:
                        gap = tensor_info[allocated_tensor].sram_start_bank - prev_start_bank
                        if gap >= bank_need and gap < smallest_gap:
                            smallest_gap = gap
                            best_start_bank = prev_start_bank
                        prev_start_bank = max(prev_start_bank, tensor_info[allocated_tensor].sram_end_bank + 1)
                ...
                if best_start_bank is None:
                    best_start_bank = prev_start_bank
                tensor_info[tensor_id].sram_start_bank = best_start_bank
                tensor_info[tensor_id].sram_end_bank = best_start_bank + bank_need - 1
                total_used_bank = max(total_used_bank, tensor_info[tensor_id].sram_end_bank + 1)
                if total_used_bank > ArchitectureFeatures.SRAM_MAX_BANK:
                    return True
                ...
                ordered_allocated_tensors = sorted(ordered_allocated_tensors, key=lambda t: (tensor_info[t].sram_start_bank, -tensor_info[t].size))
            ...
            for tensor_id in tensor_info:
                for tensor_metadata in tensor_info[tensor_id].tensors:
                    if tensor_metadata.cid in opids or tensor_metadata.pid in opids:
                        tensor_metadata.sram_start_bank = tensor_info[tensor_id].sram_start_bank
                        tensor_metadata.sram_end_bank = tensor_info[tensor_id].sram_end_bank