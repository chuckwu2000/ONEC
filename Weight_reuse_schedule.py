...
class Weight_reuse_scheduler:
    def __init__(self, graph, need_allocate_tensors, same_layer_next_opids, fragment_ratio = 0.15):
        ...
        me_input_buffer_size = math.floor(ArchitectureFeatures.ME_INPUT_BUFFER_SIZE * reserved_ratio / split_ratio)
        me_weight_buffer_size = math.floor(ArchitectureFeatures.ME_WEIGHT_BUFFER_SIZE * reserved_ratio / split_ratio)
        me_output_buffer_size = math.floor(ArchitectureFeatures.ME_OUTPUT_BUFFER_SIZE * reserved_ratio / split_ratio)
        # Element-wise engine's SRAM limits
        ee_input_buffer_size = math.floor(ArchitectureFeatures.EE_INPUT_BUFFER_SIZE * reserved_ratio / split_ratio)
        ee_output_buffer_size = math.floor(ArchitectureFeatures.EE_OUTPUT_BUFFER_SIZE * reserved_ratio / split_ratio)
        ...
        for order, op in enumerate(ordered_ops):
            ...
            # Operation that will execute in NPU
                ...
                if opcode_type in elem_wise_ops:
                    ...
                    for tensor_id in op_info['inputs'][0:input_nums[input_idx]]:
                        ...
                        if tensor_id in self.tensors_wait_consume:
                            ...
                            #   Mul
                                ...
                                self.all_buffer_size[buffer_type.EE_INPUT_BUFFER] -= self.need_allocate_tensors[tensor_id].size
                    ...
                    # Output tensor
                    for tensor_id in op_info['outputs']:
                        self.all_buffer_size[buffer_type.EE_OUTPUT_BUFFER] -= self.need_allocate_tensors[tensor_id].size
                        # Update the tensor metadata
                        for tensor in self.need_allocate_tensors[tensor_id].tensors:
                    ...
                    for tensor_id in op_info['inputs'][0:input_nums[input_idx]]:
                        ...
                        if tensor_id in self.tensors_wait_consume:
                            ...
                            if Internal_memory_area.ME_OUTPUT_BUFFER in tensor_had_allocated:
                                self.all_buffer_size[buffer_type.ME_INPUT_BUFFER] -= self.need_allocate_tensors[tensor_id].size
                        ...
                        # Load the tensor into ME_SRAM (usually is the constant tensor, ex: weight)
                            ...
                            if tensor_id == -1:
                                continue
                            self.all_buffer_size[buffer_type.ME_WEIGHT_BUFFER] -= self.need_allocate_tensors[tensor_id].size
                            # Update the tensor metadata
                            for tensor in self.need_allocate_tensors[tensor_id].tensors:
                                if tensor.cid == opid:
                    ...
                    # Output tensor
                    for tensor_id in op_info['outputs']:
                        self.all_buffer_size[buffer_type.ME_OUTPUT_BUFFER] -= self.need_allocate_tensors[tensor_id].size
                        # Update the tensor metadata
                        for tensor in self.need_allocate_tensors[tensor_id].tensors:
    ...
    def perform_weight_reuse(self, block_start_order, block_end_order):
        ...
        for op in self.graph.ordered_ops[block_start_order: block_end_order + 1]:
            print(f"ori list: op: {op.opid}")
        for op in self.graph.ordered_ops[block_start_order: block_end_order + 1]:
            # same_layer_next_opids[x]: (head_opid_of_layer, x's next_opid)
            next_opid = self.same_layer_next_opids.get(op.opid, (-1, -1))[1]
            print(f"op: {op.opid}, next_opid: {next_opid}")
            if next_opid != -1:
                ops_on_next_path.append(next_opid)
        while len(ops_on_next_path) > 0:
            print(f"ops_on_next_path: {ops_on_next_path}")
            new_list = []
            for now_opid in ops_on_next_path:
                now_op = self.graph.ops[now_opid]
                next_opid = self.same_layer_next_opids.get(now_op.opid, (-1, -1))[1]
                print(f"now_opid: {now_opid}, next_opid: {next_opid}")
                if next_opid != -1:
